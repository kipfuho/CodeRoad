...
int temp_arr[86401], prefixsum[86401], total_error[10000], prob[10000][100], total_point[10000];
...
int main(){_
    ...
    while(cin >> token1){
        ...
        else if(prob[user_id][prob_id] < token5){
            total_point[user_id] += (token5 - prob[user_id][prob_id]);
            prob[user_id][prob_id] = token5;
        }
    ...
    while(cin >> token1){
        ...
        if(token1 == "?total_number_submissions"){
            ...
            cout << total_error[user_id] << endl;
        }
        else if(token1 == "?total_point_of_user"){
            cin >> token2;
            user_id = get_number(token2);
            cout << total_point[user_id] << endl;
        }
